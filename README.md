...
5. Add your `AI_GATEWAY_API_KEY` to your Vercel environment variables
6. When your deployment has finished, open your App Manifest from the Slack App Dashboard
7. Update the manifest so all the `request_url` and `url` fields use `https://<your-app-domain>/api/slack/events`
8. Click save and you will be prompted to verify the URL
9. Open your Slack workspace and add your new Slack Agent to a channel. Your Slack Agent should respond whenever it's tagged in a message or sent a DM
   - _Note_: Make sure you add the production app, not the local app we setup earlier
10. Your app will now automatically build and deploy whenever you commit to your repo. More information [here](https://vercel.com/docs/git)
